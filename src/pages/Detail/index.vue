        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 
                  以前的路由跳转: 从A路由到B路由, 这里在加入购物车进行路由跳转之前,
  ...
  export default {
    ...
    methods: {
      ...
      changeActive(saleAttrValue, valueArr){
        ...
        saleAttrValue.isChecked = "1"; // 点击的高亮
      },
      // 表单元素修改产品个数
      changeSkuNum(event){
        // 用户输入的文本*1
      ...
      async addShopcar(){
        ...
        // 2.服务器存储成功, 进行路由跳转传递参数; 存储失败, 给用户提示
        /* 
          当前这里派发了一个action, 向服务器发请求了, 判断加入购物车是成功还是失败, 进行相应的操作
        */
        try {
          await this.$store.dispatch("addOrUpdateShopCart", {skuId: this.$route.params.skuid, skuNum: this.skuNum});
          // 一些简单的数据skuNum, 通过query形式给路由传递过去
          // 产品信息的数据skuInfo, 比较复杂, 通过会话存储(不持久化, 会话结束了消失)
          // 本地存储/会话存储一般存储的是字符串
